...
3. VRAM 효율성 극대화: 이제 모드(Base <-> LoRA)를 전환해도 거대 모델이 메모리에 중복으로 올라가서 터지는(OOM) 현상 차단.
"""
...
class QwenClient:
    def __init__(self, base_model_path: str = "Qwen/Qwen2.5-7B-Instruct", adapter_path: Optional[str] = None):
        """
        [클래스 초기화]
        객체가 생성될 때 경로 이름만 변수에 저장하고, 실제 무거운 모델 다운로드나 로딩은 아직 하지 않음 (지연 로딩을 위한 준비)
    ...
    def _load(self) -> None:
        ...
        실제 첫 번째 질문(request)이 들어왔을 때 딱 한 번만 실행되어 모델을 VRAM(GPU)에 적재
        """
        
        # 방어 코드: 이미 모델이 VRAM에 올라와 있다면(None이 아니라면), 
        # 아래 무거운 로딩 과정을 쳐다보지도 않고 즉시 통과(return). 추론 속도를 향상
        ...
        try:
            ...
            # 1. 토크나이저(단어 사전) 로드
            # trust_remote_code=True: Qwen 모델 특유의 커스텀 파이썬 코드를 실행하도록 허용
            ...
            if self.adapter_path:
                ...
                # 핵심: VRAM에 올라간 base_model 객체를 그대로 둔 채, 
                # adapter_path 폴더 안의 가중치만 가져와서 추가
                self._model = PeftModel.from_pretrained(base_model, self.adapter_path)
            else:
                # 어댑터 경로가 없다면(모드 A, B), 그냥 순수 베이스 모델을 최종 모델로 사용
    ...
    def request(
        ...
        # Qwen 모델이 좋아하는 형식으로 텍스트를 자동 포맷팅합니다.
        ...
        import torch
        # 5. 추론 실행 (기억 봉인)
        # torch.no_grad(): 현재 학습 단계가 아니므로 Gradient를 저장하지 않음
        with torch.no_grad():
